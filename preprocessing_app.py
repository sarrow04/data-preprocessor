# -*- coding: utf-8 -*-
"""
preprocessing_app_v20_fix
f-stringの閉じ忘れによるSyntaxErrorを修正した最終版
"""
    ...
    with st.expander("日付型への変換"):
        ...
        if st.button("日付型に変換を実行", key=f"btn_date_{selected_column}"):
            try:
                ...
                temp_series = df_copy[col_name].copy()
                pre_missing = temp_series.isnull().sum()
...
def display_feature_engineering(df):
    ...
    with st.expander("ワンホットエンコーディング"):
        ...
        if st.button("ワンホットエンコーディングを実行"):
            if ohe_cols:
                st.session_state.df = pd.get_dummies(df, columns=ohe_cols, dtype=float); st.success("ワンホットエンコーディングを実行しました。"); st.rerun()
...
def display_variable_settings(df):
    ...
    if st.button("変数の役割を設定"):
        if selected_target != "---" and len(selected_features) > 0:
            st.session_state.target_col = selected_target; st.session_state.feature_cols = selected_features
